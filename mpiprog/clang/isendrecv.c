...
int main(int argc, char **argv) {
    ...
    for (size_t iter = 0; iter < size - 1; iter++) {
		...
		//MPI_Isend(nA, mrow*ncol, MPI_DOUBLE, (rank == 0 ? size - 1 : rank - 1), 11,
		//	MPI_COMM_WORLD, &request);
		//MPI_Irecv(nA, mrow*ncol, MPI_DOUBLE, (rank == size - 1 ? 0 : rank + 1), 11,
		//	MPI_COMM_WORLD, &request);

		MPI_Isendrecv(nA, mrow*ncol, MPI_DOUBLE, (rank == 0 ? size - 1 : rank - 1), 11,
					  nA, mrow*ncol, MPI_DOUBLE, (rank == size - 1 ? 0 : rank + 1), 11,
					  MPI_COMM_WORLD, &request);
