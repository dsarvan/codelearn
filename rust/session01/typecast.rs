fn main() {
    ...
    let word: u16 = 128; // 16-bit unsigned integer
    let byte = word as i8; // 8-bit signed integer
    let ubyte = word as u8; // 8-bit unsigned integer
    dbg!(word, byte, ubyte);
...
/// float equality (hack! - does not handle NaN/Infinity,
